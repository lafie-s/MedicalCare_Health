...
网站维护访问配置沿用维护/版本对话框契约：管理员变更，所有角色只读回显，显式版本冲突，草稿关闭确认，过期状态提示。空白名单的全访客阻止含义在编辑与确认两处呈现。


故障日志查询支持加载、无记录、失败后重试、30 秒过期提示、授权环境隔离；只读角色可查。


自动重启默认关闭，管理员启用需原因和确认；未知执行状态必须按任务核对解锁。执行/采样状态每 5 秒读取，确认或草稿对话框暂停读取。模式和边界持续显示，demo 按钮不出现在真实执行器模式。
