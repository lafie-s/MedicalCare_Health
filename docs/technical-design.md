...
F06a 已实现单服务 HTTP 连续失败规则和平台内事件处理，支持 warning/critical 等级、自动恢复、人工确认和恢复后关闭。无数据不恢复，配置变更终止并保留原因。事件保存规则快照，评估与采样完成及审计同事务。具体规则、权限、边界见 `docs/http-alerts.md`。通知、持续时间规则、维护静默及告警中心仍待后续实施；健康概览的探测状态与告警事件等级分别展示。
