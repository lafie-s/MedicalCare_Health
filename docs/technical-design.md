...
F07c 网站维护访问控制：新增 IP/CIDR 白名单、立即维护/跟随窗口/关闭模式、带版本幂等及原子审计的管理员配置接口、独立入口鉴权与 503 维护页。正式业务入口须部署鉴权及排空长连接；公开演示仅连接独立访客入口。详见 docs/maintenance-access.md。


F06c（2026-09-09）新增独立 failure_logs：探测完成时与审计同事务保存 HTTP 异常、超时、连接错误码及中断记录。30 天、每服务最多 10000 条，与一天探测样本清理分离；不保存响应正文、凭据或患者数据。


F07d（2026-09-09）：连续故障自愈采用自动采样触发、3 次门槛、10 分钟冷却和每小时 3 次上限。记录诊断后重启批准 Docker 容器，容器及 HTTP 检查通过才成功；不确定结果锁定。发布和恢复在同一状态库事务内互斥，维护暂停。详见 docs/automatic-recovery.md。
