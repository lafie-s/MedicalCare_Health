...
| F01 基础与权限 | 4 | M0 | 可启动工程、登录与环境权限验证通过 | 进行中：F00 与 F01a 已实现；登录页面、真实员工联调待完成 |
| F01a 身份交换与环境授权 | 1（提前实施，计入 F01 原估算） | F00、已知身份协议 | 会话撤销、越权拒绝、异常及持久化验证 | 已实现，上传结果见交付记录 |
| F02 服务目录 | 2 | F01 | 服务配置、校验、授权与停用可用 | 待开始 |
...
F00、F01a 的 `done` 表示代码及对应自动验证完成，Git 远程上传状态单独记录。F00 提交 `7d9fd87` 推送时 GitHub 连接失败。M0、M1、M2、M3 均未达到完整验收；当前未接入真实监控数据。

下一步：完成登录交互与真实员工授权联调，再实现 F02 服务目录及 F03 健康探测。F01a 未替代 F01 完整验收，提前实施项不增加原先 38 工作日估算。
