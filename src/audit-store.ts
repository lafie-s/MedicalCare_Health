...
export interface AuditQuery { environmentId: string; from: number; to: number; page: number; category: "all" | "service" | "probe" | "alert" | "maintenance" | "release" | "recovery" }
...
const scoped = `WITH parsed AS (
  ...
  FROM resources a JOIN services s ON s.id = CASE
    WHEN a.operation IN ('recovery.enabled','recovery.disabled','recovery.started','recovery.succeeded','recovery.canceled','recovery.unknown','recovery.resolved','recovery.simulated','maintenance.access_saved','service.created','service.updated','probe.started','alert.rule_saved','release.started','release.succeeded','release.failed','release.unknown') THEN a.resource_id
