...
export function ServicePanel({ environmentId, role, onUnauthorized }: { environmentId: string; role: string; onUnauthorized: () => void }) {
  ...
  const modalOpen = useRef(false); modalOpen.current = Boolean(edit || toggle || trend || alerts || maintenance);
  ...
  </section><AlertCenter environmentId={environmentId} revision={0} paused={Boolean(edit || toggle || trend || alerts || maintenance)} onUnauthorized={() => auth.current()} onOpen={(id) => { const service = data?.items.find((item) => item.id === id); if (service) setAlerts(service); else { setError("服务目录暂不可用，请刷新服务后重试"); } }} /></>;
}
