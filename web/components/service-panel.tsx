...
export function ServicePanel({ environmentId, role, launcher, onCloseLauncher, onUnauthorized }: { environmentId: string; role: string; launcher: "release" | "access" | "recovery" | null; onCloseLauncher: () => void; onUnauthorized: () => void }) {
  ...
  const [recovery, setRecovery] = useState<Service | null>(null);
  ...
  const modalOpen = useRef(false); modalOpen.current = Boolean(edit || toggle || trend || alerts || maintenance || release || launcher || access || failure || recovery);
  ...
  </section><AlertCenter environmentId={environmentId} revision={0} paused={Boolean(edit || toggle || trend || alerts || maintenance || release || launcher || access || failure || recovery)} onUnauthorized={() => auth.current()} onOpen={(id) => { const service = data?.items.find((item) => item.id === id); if (service) setAlerts(service); else { setError("服务目录暂不可用，请刷新服务后重试"); } }} /></>;
}
