...
export function Console() {
  ...
  const [launcher, setLauncher] = useState<"release" | "access" | "recovery" | null>(null);
  ...
  return <div className="console-layout"><aside className="sidebar"><Brand /><p className="nav-label">工作空间</p><span className="nav-current">授权环境</span><div className="sidebar-bottom"><p>MedicalCareWeb</p><span>运行指标监测及维护</span></div></aside>
    <main className="workspace"><header className="workspace-header"><span>运维工作台 <span className="muted">/ 授权环境</span></span><div className="user-actions"><span>{state.user.displayName}<small>{roles[state.user.role]}</small></span><Button busy={busy} onClick={() => void logout()}>退出登录</Button></div></header>
      <div className="workspace-content">{process.env.NEXT_PUBLIC_PREVIEW === "1" && <Notice>隔离演示 · 所有数据均为示例，未连接真实业务系统；修改会在重启后重置。</Notice>}<div className="page-heading"><div><p className="eyebrow">工作空间</p><h1>选择运维环境</h1><p className="muted">仅展示你获准访问的环境。进入前请核对环境类型。</p></div><div className="inline-actions">{environment && <><Button onClick={() => setLauncher("recovery")}>自动重启</Button><Button onClick={() => setLauncher("access")}>网站维护</Button><Button className="primary" onClick={() => setLauncher("release")}>网站版本更新</Button></>}<Button onClick={() => void load()}>刷新环境</Button></div></div>{error && <Notice error>{error}</Notice>}
        ...
        {environment && <ServicePanel key={`services-${environment.id}`} environmentId={environment.id} role={state.user.role} launcher={launcher} onCloseLauncher={() => setLauncher(null)} onUnauthorized={() => { active.current?.abort(); setState({ kind: "login", message: "会话或权限已失效，请重新登录" }); }} />}
        {environment && state.user.role === "admin" && <AuditPanel key={`audit-${environment.id}`} environmentId={environment.id} onUnauthorized={() => { active.current?.abort(); setState({ kind: "login", message: "会话或权限已失效，请重新登录" }); }} />}
